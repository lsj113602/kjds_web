...
function GetLoadNav() {
    ...
    $.each(data, function (i) {
        ...
        if (row.PID == "0") {
            _html += '<li>';
            _html += '<a data-id="' + row.ID + '" href="#" class="dropdown-toggle"><i class="' + row.Icon + '"></i><span>' + row.AuthorizeName + '</span><i class="fa fa-angle-right drop-icon"></i></a>';
            var childNodes = row.ChildNodes;
            if (childNodes.length > 0) {
                _html += '<ul class="submenu">';
                $.each(childNodes, function (i) {
                    var subrow = childNodes[i];
                    subrow.UrlAddress = "/Admin" + subrow.UrlAddress;
                    _html += '<li>';
                    _html += '<a class="menuItem" data-id="' + subrow.ID + '" href="' + subrow.UrlAddress + '" data-index="' + subrow.SortCode + '">' + subrow.AuthorizeName + '</a>';
                    _html += '</li>';
                });
                _html += '</ul>';
            }
