...
namespace PLKJDS.Data
{
    public interface IRepositoryBase : IDisposable
    {
        ...
        List<TEntity> FindList<TEntity>(Expression<Func<TEntity, bool>> predicate, string strSql, Pagination pagination) where TEntity : class;
        List<TEntity> FindList<TEntity>(Expression<Func<TEntity, bool>> predicate, Pagination pagination) where TEntity : class,new();
        List<TEntity> FindList<TEntity>(string strSql,Pagination pagination) where TEntity : class;
        List<TEntity> FindList<TEntity>(string strSql, DbParameter[] dbParameter, Pagination pagination) where TEntity : class;
